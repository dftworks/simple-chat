...
async fn check_server_connection(port: u16) -> bool {
    for _ in 0..20 {
        // Retry for 10 seconds (20 attempts with 500ms delay)
...
async fn print_server_output(
    server_stdout: tokio::process::ChildStdout,
    server_stderr: tokio::process::ChildStderr,
) {
