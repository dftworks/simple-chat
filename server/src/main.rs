...
struct AppState {
    clients: Mutex<HashMap<String, broadcast::Sender<ChatMessage>>>,
...
async fn handle_socket(socket: WebSocket, state: Arc<AppState>) {
    ...
    let mut receiver = state.sender.subscribe(); // Subscribe to broadcast channel
    ...
    if let Some(Ok(msg)) = ws_receiver.next().await {
        if let Message::Text(name) = msg {
            ...
            clients.insert(username.clone(), state.sender.clone());
        }
    }
